...
for key, fun in distances.items():
    ...
    n_in_value, n_out_value, w_in_value, w_out_value, w_in_separate_values, w_out_separate_values, volumes = \
        all_internal_values(data, labels_predict)
    ...
    if len(np.unique(labels_predict)) == n_clusters:
        internal['DB'] = davies_bouldin_measure(data, labels_predict)
        internal['S'] = silhouette_mean_measure(data, labels_predict)
    else:
        internal['DB'] = 10000.0
        internal['S'] = 0
